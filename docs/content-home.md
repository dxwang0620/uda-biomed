...
| 項目 | 做法 |
| --- | --- |
| 區塊寬度 | `max-width: 64rem`。研發卡片搬走後這一塊變成滿版 1440px，摘要橫跨整個畫面、右下的按鈕離文字非常遠（回報「太寬了」） |
| 標題與清單的間距 | `.blockTitle` 補 `margin-bottom: var(--space-3)`（實測 24px），標題不再黏在第一則上 |
| 每則右下的按鈕 | `Button variant="outline" size="sm"`，文字 `Read more` |

按鈕全部指向 `/research`：單則消息沒有自己的頁面（純前端靜態站，也還沒有內容
來源），所以指向同一頁，而不是編造一個不存在的網址。九顆按鈕的可見文字相同，
讀屏逐一唸過會分不出差別，各自補了一段 `visually-hidden` 的標題。

...
**整張卡由上到下只有一條漸層**（指定），同一個色票、只有濃度在變：
實色 → `0.96`（48%）→ `0.78`（64%）→ `0.36`（82%）→ `0.1`（100%）。
照片就從色塊裡延伸出來。
...
| 拆成「文字一層 ＋ 照片一層」 | 對比度好算，但兩層的色標對不起來，看起來是兩塊拼接 |
| 色票 → navy 雙色漸層 | 對比度更好，但不是要的效果 |
| **一條跨滿整張卡**（現在） | 文字實測落在卡片 43～50%，該處濃度 0.96 以上 |

白字在最不利的情況（0.96 濃度、最亮的 01 色票、純白照片）最差是眉標 5.69:1。

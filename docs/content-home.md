...
判斷「走過幾顆」時帶 **2% 的容差**，不是浮點誤差的緩衝：捲到最後一張時
`scrollLeft` 常常停在 656.5 / 657 這種位置（snap 對齊、裝置像素比、慣性收尾
都會差一點），進度永遠差一點到 1，第三顆就不會亮——回報的
「已經滑到第三張，圓點還沒到」就是這個。
...
肖像原圖是橫幅（人物偏左、右側是 UDA 招牌），裁成版型要求的 3:4 一定會切掉一邊，
`object-position: 34% 22%` 往左上取景，保住人物與桌上的「董事長 Chairman」名牌。
肖像欄也從 8rem 放寬到 11rem——8rem 對佔位框夠用，放真照片時人臉太小。

...
先取 0.78，後來依指示再降一階（回報「擋到太多背景」）到 **0.70**——
**這是地板**，再降到 0.65 就掉到 4.34、低於 AA。手機那組同時從 0.95 降到 0.80
（約 6.4:1）；留得比桌機厚一點，是因為沒有模糊時影片細節會清晰透出，
字壓在雜訊上比壓在同亮度的平面上難讀。`backdrop-filter: blur(10px)` 純粹是質感——

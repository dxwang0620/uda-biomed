...
兩個主題標題（Solution Analysis / Latest News）也轉藍，但用的是**淺藍 `#a8c8ff`**。

> ⚠️ 它們在這片玻璃上是白字，底下直接是背景影片，不是卡片內的淺色區。
> 第一版跟著換成 `--color-primary` 那種深藍，實測整行明顯變暗、看起來像消失；
> 第二版乾脆不讓它們變色，結果是「hover 變藍」在最顯眼的那一行看不到。
> 淺藍是第三版：亮度層級維持在白字那一階，顏色看得出變了。

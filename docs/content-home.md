...
| | 順序 |
| - | ---- |
| 窄螢幕 | hero → 消息 → 研發卡片 → 董事長談話 → **四格數字** → 技術平台 → … |
| ≥1024 | hero → 消息｜董事長談話 → **四格數字** → 研發卡片 → 技術平台 → … |

四格數字因此不再自成一個 Section，而是放進 `.newsFocus` 這個容器裡——
**跨 Section 沒辦法指定它要排在哪一列**。寬螢幕靠 grid 明確指定列序
（第一列消息｜董事長、第二列四格、第三列卡片），窄螢幕照 DOM 順序。

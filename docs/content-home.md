...
兩者不一樣，因為卡片列右側留了下一張的邊角：第三張卡佔滿畫面時，捲動比例
其實只有七、八成。用比例判斷的話，第三顆要等捲到最底才會亮——回報的
「已經滑到第三頁，底下滑軌還沒到指定位置」就是這個落差。

實測（1920 寬、可捲 990）：

| scrollLeft | 三張卡在畫面中的寬度 | 佔最多 | `--p` | 圓點 |
| --------- | ---------------- | ---- | ---- | ---- |
| 700 | 0 / 532 / 318 | 第 2 張 | 0.707 | 2 |
| 900 | 0 / 332 / 518 | 第 3 張 | 0.909 | **3** |

> 實作上的坑：`offsetLeft` 是相對於**定位祖先**（這裡一路到 `body`），
> 不是相對於捲動容器。實測三張卡是 814 / 1438 / 2062，而 `scrollLeft` 從 0 起算——
> 不扣掉卡片列自己的偏移，三張的座標都比可視區大好幾百，永遠算成第一張。

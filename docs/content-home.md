...
指定「solution analysis 變三格」「裡面的圖片高度不用太高」。
...
- **每一則矮一點、而且等高**（指定「圖片等高」）：摘要收到三行（原本最長五行）、
  標題收到兩行、內距 24→16px、箭頭上方留白 16→8px，再給一個
  `min-height: 13.5rem`（216px）。375px 實測五則全是 **222px**，箭頭都落在距底 16px。
  箭頭維持 44×44 不動——那是觸控目標的下限。
- 箭頭要沉到列底，`.newsBody` 得跟著列一起拉高：`grid-template-rows: auto auto auto 1fr`
  ＋ `align-content: stretch`（基準規則是 `start`，會把四列全擠在上方）。

> ⚠️ **等高是往矮的那一階對齊。** 一度改成對齊最高的那一則（`15.5rem`／248px），
> 好處是標題不用截斷，但那是把每一則都拉高，回報「太高了」。
> 代價：最新消息那兩則的標題與摘要會被 `line-clamp` 截斷並補上刪節號
> （`Cancer detection research: where t…`）。那兩則的文案本來就是草稿，
> 換成短一點的標題就不會出現刪節號。

> ⚠️ 手機版的照片是鋪滿整列的背景，所以「照片的高度」就是那一列的高度，
> 而列高完全由文字決定。收高度只能從文字下手，**不能給照片一個固定高度**——
> 那會讓文字溢出卡片。
>
> ⚠️ 一度把整塊收窄到 `88% / 30rem`，讓卡片從背景裡浮起來，
> 但回報「整個區塊回復跟最初一樣大、不用縮減」，已移除，維持滿版（375px 下 339px）。

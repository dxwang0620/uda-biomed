...
### 消息區塊的間距與對齊（2026-09-16）

- **兩個主題標題置中**（指定）：`Solution Analysis` 與 `Latest News`。
- **`Latest News` 上方的 padding 拿掉**（原本 `--space-4`）。分隔線與 48px 的
  `margin-top` 都留著，段落感由 margin 負責，標題直接接在線下方。
- **外框上下內距各加一階**：桌機 `--space-4` → `--space-6`（32→48），
  手機 16 → 32。左右不動。

### 手機版的消息區塊

指定「solution analysis 變三格」「比例跟上面那三格數字一樣」。

- **只列三則，所有寬度都一樣**（指定「電腦版也要保持三個」）：
  `.newsListTrim > li:nth-child(n + 4) { display: none }`，規則下在基準層而不是
  手機的 media query 裡。資料仍是六則，第四則之後不算進無障礙樹。
  不在 React 端切資料——那要多一個 matchMedia 狀態，而且預抓的 HTML 沒有視窗寬度，
  會先畫六則再閃成三則。
- **每一則就是一張 21:9 的卡**（2026-09-16，比照首頁那三格數字）。
  手機版的照片是鋪滿整列的背景，所以「圖片的長寬比」就是這一列的長寬比，
  給 `aspect-ratio` 同時解決兩件事。`min-height` 要一起歸零，否則兩者取大值。
- **外框內距 32 → 16px**（指定）。每一則因此寬 32px，21:9 之下高度也多約 14px。
- 375px 實測：外框 341px、每一則 **284×122（比例 2.33）**，五則完全一致，無橫向捲軸。

> ⚠️ **21:9 之下高度很緊。** 一度整段收掉摘要，但回報「裡面的文案不見了」。
> 改成內距由 16px 收到 12px、摘要收到一行，就排得進去：
> 實測內容高 95px，卡片 121px，還有 14px 餘裕（分類 20 ＋ 標題兩行 41
> ＋ 摘要一行 20 ＋ 列距 8 ＋ 上下內距 24）。
> 箭頭改成絕對定位壓在右下角——留在文件流裡會佔掉一整列的高度，
> 122px 裡放不下分類、標題再加一顆 44px 的鈕。整則因此多一段 56px 的右內距，
> 文字不會跑到箭頭底下。箭頭本身維持 44×44，那是觸控目標的下限。
>
> 走過的版本：等高 `min-height: 13.5rem`（222px，摘要三行）→ 21:9 但無摘要
> → 現在的 21:9 ＋ 一行摘要。
>
> 桌機（≥768）完全不受這些影響：仍是兩欄、160px 縮圖在左、摘要照常顯示，
> 1100px 實測每一則 450×247、`aspect-ratio: auto`。

> 這幾條刻意用 `max-width: 767px`，與檔案其他地方的 mobile-first 相反。
> 寫成基準值就得在 768 再寫一組還原，而還原 `display` 要把 `.newsItem` 的
> `grid` 值抄第二份——抄錯就壞。位置也必須留在 `Home.module.css` 最後：
> 與前面的 `.newsBlock`／`.newsItem` 同權重，CSS Modules 依原始順序輸出，後面的才蓋得掉。
...
依指示把原本的 To add 方塊改成正式內容：標籤從 `To add` 換成編號 `01／02／03`，
內容換成文案。

**虛線框後來也依指示移除了**（2026-09-11）。那條虛線是 `Placeholder` 元件用來標示
「這裡還沒有內容」的記號，內容換成正式文案之後留著只會讓人以為還沒做完。
左右內距一併拿掉——沒有框之後，內距只會讓三欄的文字對不齊。
...
| | 順序 |
| - | ---- |
| 所有寬度 | hero → **核心定位 ＋ 四格數字** → 企業責任 → **技術平台** → 消息區塊 |
...
- **企業責任與消息區塊對調**（2026-09-08）。消息與技術平台導言同屬一個
  Section（`.newsFocus`），技術平台一開始跟著消息一起往下移
- **技術平台移到消息區塊上面**（2026-09-08）。容器內的順序對調即可；
  上緣間距跟著換手——原本帶間距的是技術平台，現在是消息區塊
  （`.newsFocus > .newsBlock`）
- **首頁的聯絡區塊整段刪除**（2026-09-07）。`GET IN TOUCH ／ Two ways to reach us`
  那段文案依指示搬到 CONTACT 頁，見 `docs/content-contact.md`
...
- 「最新消息」底下兩則（`LATEST`）。**前兩則依指示改成實際的內容並各自連到內頁**（2026-09-11）：

  | | 主題 | 去向 | 縮圖 |
  | - | --- | --- | --- |
  | 1 | 癌症檢測研究 | `/research` | `news-detection` ← `index_img/news/news1.jpg` |
  | 2 | AFL 嗅覺語言感測晶片 | `/research/digital-health` | `news-afl` ← `index_img/news/news2.jpg` |

  縮圖是你另外提供的兩張（960px 寬，jpg 65～82K、webp 45～61K）。
  **第三則依指示刪除**，這一組現在只有兩則。
  舊的 `news-1`～`news-5` 辦公環境照仍由「解決方案分析」那一組輪用，
  但**該組第一則依指示改用 `news-lobby`**（品牌牆與展示中的晶片，960px 寬、
  jpg 86K／webp 62K）。

  `NewsItem` 因此多了三個可選欄位：`to`（覆寫去向）、`thumb` / `thumbAlt`
  （覆寫縮圖）。沒給的項目維持原本的行為——指向 `/research`、輪用辦公環境照。
...
| 數字統計格 | `--glass: 0.25` ＋ `blur(4px)`（與玻璃卡同值） | `--glass: 0.55`，無模糊 |
...
先取 0.78，後來依指示降到 **0.70**——**平鋪白膜的地板就是這裡**，
再降到 0.65 就掉到 4.34、低於 AA。

再一次回報「還是遮到太多」之後改了做法：**白膜不平鋪，改成局部橢圓**。
文字底下仍是 0.70（對比不變），卡片邊緣一路淡到 0.10，影片透得出來。
手機同理，中心 0.80、邊緣 0.14。

橢圓的尺寸是量出來的：文字佔卡片 y 33%～75%、x 約 28%～72%；
中心 50%/54%、半徑 96%/92% 之下，文字最外緣的正規化距離是 0.32，
還在 0.70 那個色標（38%）之內。

> ⚠️ **橢圓要大到超出卡片、色標一路拉到 100%。** 第一版用 62%/58% ＋
> 70%→88% 收尾，等於在卡片中間畫了一個邊界清楚的白色橢圓，看起來像污漬。
> 放大之後淡出的終點落在卡片外，卡片內看到的才是連續的過渡。

最後依指示**直接對齊底下那張玻璃卡的濃度**（`.card` 的 `--glass`）：
桌機 0.25 ＋ `blur(4px)`、手機 0.55、外框與陰影一併比照，橢圓白膜與藍色濃度
都跟著降。文字色同時從 `--color-primary`／`--color-text` 換成
`--color-text-on-glass`（#05101f）——玻璃卡用的就是這個色，比原本深，
同一片白膜下對比高一階。

> ⚠️ **桌機這個濃度在最暗的影片畫面上不合格。** 0.25 白膜是 1.96:1，
> 遠低於 AA 的 4.5:1。那張玻璃卡本來就是這個狀態（當初指定的視覺方向），
> 這一排現在跟著它走，等於把同一個例外擴大到四格數字上。
> 手機那組（0.55、無模糊）是 5.69:1，合格。
>
> 先前的版本：平鋪 0.70（4.93:1，合格）→ 局部橢圓（文字底下 0.70，邊緣 0.10）。
> 要回到合格狀態，把 `--glass` 調回 0.70 即可。`backdrop-filter: blur(10px)` 純粹是質感——
...
依指示比照四格數字（「跟全球人口那樣」），但**拿掉翻轉**，也拿掉那邊的深藍填色
與白字——這一塊裡有九則深色文字與九張縮圖，底下換成深藍之後字會直接消失；
四格只有三行字才做得起來。留下的是浮起（`translateY(-6px)`）、陰影與掃光。

`scale` 只用 **1.005**：這一塊在寬螢幕是滿版 1440px，四格用的 1.02 會左右各長出 14px。

> ⚠️ **hover 的 `transform` 必須加 `!important`。**
> 這一塊同時掛著 `.reveal` 的捲動進場動畫（`animation-fill-mode: both`），
> 它最後一個關鍵影格是 `transform: none`，而**動畫的宣告會蓋過一般宣告，
> 與特異性無關**。不加的話 hover 位移完全不生效——實測 `transform` 停在
> `matrix(1,0,0,1,0,0)`，只有陰影會變（`box-shadow` 不在 reveal 的動畫裡）。
> 階層順序：transition > `!important` > animation > 一般宣告。
>
> 實測：靜止 `matrix(1,0,0,1,0,0)` ＋ 陰影 `rgba(0,24,56,0.18) 0 8px 32px`；
> hover `matrix(1.005,0,0,1.005,0,-6)` ＋ 陰影 `rgba(0,48,104,0.28) 0 18px 38px`。
...
兩個主題標題（Solution Analysis / Latest News）用 **header 的藍
`--color-navy` `#003068`**（依指示）——就是 header 白底狀態下的字色。

改過四版，記在這裡免得又繞回去：

| 版本 | 標題 hover 色 | 結果 |
| --- | --- | --- |
| 1 | `--color-primary` `#0f3661` | 整行明顯變暗，像消失 |
| 2 | 不變色（維持白） | 最顯眼的一行看不出「變藍」 |
| 3 | 淺藍 `#a8c8ff` | 亮度層級維持，但依指示不要淺藍 |
| **4（現行）** | **`--color-navy` `#003068`** | 依指示 |

> ⚠️ 第 4 版與第 1 版是同一類：**深色字壓在半透明玻璃上，而玻璃底下是背景影片**，
> 影片暗的時候會糊進背景。這是指定的取捨，不是疏漏。

### 手機的按壓狀態

觸控裝置沒有 hover，上面那組效果整個關在 `@media (hover: hover)` 裡，
手機上點下去完全沒反應（回報「手機版好像有辦法一樣有點擊改變的特效嗎」）。

做法與四格數字相同：用 pointer 事件加上 `.pressed`，效果與 hover 那組一致
（浮起、陰影、掃光、文字轉藍）。四格那邊的 `usePress` 已抽到
`src/hooks/usePress.ts`，兩處共用同一份，不複製第二份。

- 用 `pointerdown` 而不是 `:active`——`:active` 在 iOS Safari 上對非互動元素
  不一定觸發，不能依賴
- 放開後至少維持 260ms 再收：手指點一下往往不到 100ms，不留最短顯示時間
  效果會一閃而過
- `pointercancel` 會清掉狀態，所以手指落下後改成捲動時不會卡住不放
- `.pressed` **不能**跟 hover 那組合併成一組選擇器：hover 那組必須留在
  `@media (hover: hover)` 裡（觸控裝置上 `:hover` 點過會黏著），
  而 `.pressed` 要在所有裝置都生效
- `transform` 同樣要 `!important`，理由與 hover 那條相同（`.reveal` 的動畫）

實測（合成 touch 事件）：按下 → `pressed` 類別出現、`matrix(1.005,0,0,1.005,0,-6)`、
標題與內文往藍色過渡；放開 → 全部復原。

## 數字統計：從四格變三格

「Patent applications／專利申請」依指示移除（2026-09-11），剩三格。

排法仍是**所有寬度橫排兩格**（指定），所以第三格會單獨佔半排、右邊空一塊。
加了一條 `.cell:last-child:nth-child(odd) { grid-column: 1 / -1 }` 讓奇數個時
最後一格跨滿整列——用 `:last-child:nth-child(odd)` 而不是寫死第 3 格，
之後增減項目會自動適應，也不必為了湊數補一個佔位格。

> ⚠️ **剩下的「Programmes in development／研發中項目數」（12）仍是佔位值**，
> 畫面上沒有任何標記說明。前兩格（全球人口 8.3B、每年新增癌症病例 20M）是
> 公開的概略數字，這一格不是。上線前要嘛換成真實數字，要嘛一併移除。

## Hero 輪播的圓點：做成滑軌

依指示改成「像滑軌一樣」：一條橫線串起五顆點，橘色標記**滑**到當前那一顆
（2026-09-11）。語彙與研發焦點的進度軌一致。

作法：

- `.dots` 上掛一個 inline 的 `--dot-i`（當前索引），標記的位置由它算出來：
  `transform: translateX(calc(var(--dot-i) * 32px))`。切換時 CSS 自己補間，
  **JS 不碰像素、不跑動畫**
- 用 `transform` 而不是 `left`，位移才走合成器、不觸發版面重算
- 軌道左右各內縮 16px（半個熱區），兩端剛好停在第一顆與最後一顆的圓心上
- 當前那一顆自己不上色（`.dotActive::before` 設 transparent）——橘點由會滑動的
  標記負責，兩個都畫的話切換時會看到兩顆橘點
- `prefers-reduced-motion: reduce` 時關掉 transition，位置仍正確

...
| ≥1024 | hero → 消息 → **四格數字** → 董事長談話 → 研發卡片 → 技術平台 → … |

消息、研發卡片、四格數字、董事長談話四塊同屬一個 `.newsFocus` 容器。
**跨 Section 沒辦法互換順序、也沒辦法指定誰排在哪一列**，所以只要還會調順序，
它們就得待在同一個容器裡。

董事長談話原本與消息並排成兩欄，依指示獨立出來移到四格下方。
**連帶的結果是消息區塊在寬螢幕變成滿版**（1136px，原本是兩欄的 6fr ≈ 520px）——
兩欄版型存在的理由就是要並排那兩塊，其中一塊離開之後另一塊沒有理由只佔一半。
四塊現在都是滿版堆疊，兩個斷點的差別只剩順序（用 `order` 指定）。

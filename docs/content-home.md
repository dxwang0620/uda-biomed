...
| 數字統計格 | 白色橢圓（中心 70%）＋ `blur(10px)` | 白色橢圓（中心 80%），無模糊 |
...
還想更透的話，剩下的手段不是調透明度了：縮小卡片本身（少擋一點面積），
或整個拿掉白膜、數字直接壓在影片上靠文字陰影撐——後者在亮畫面上會不合格，
與 hero 標題面對的是同一個問題。`backdrop-filter: blur(10px)` 純粹是質感——

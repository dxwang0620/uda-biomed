...
淺色圓角格、大字數字在上、小標在下，捲進畫面才開始跑數。
**四格到底，手機也不換行**（指定），所以字級全部用 `clamp` —— 375 寬時
每格只有 81px，數字自動縮到 22px。
...
| 3 | 研發中項目數 | **待補** | UDA 內部數據 |
| 4 | 專利申請 | **待補** | UDA 內部數據 |

> ⚠️ **第 3、4 項沒有填數字，是刻意的。** 那是 UDA 自己的研發與專利數據，
> 我沒有來源。依 CLAUDE.md 工作方式第 3 條不編造研發成果與專利——
> 填錯一個專利件數，對外就是不實陳述。
> 畫面上顯示破折號（不是 0，0 會被當成「真的是零」），拿到數字後把
> `STATS` 裡的 `value: null` 換成數字即可，動畫會自動接上。

出處寫在區塊下方。生醫網站放數字卻不說來源，等於要人憑信任接受。
...
首頁背景是影片，亮度不可控。實測：深灰字最差 **1.46:1**、
白字在影片最亮時是 **1.00:1**，兩種都不行。所以 `.sources` 自帶白底，
深灰字在白底上是 5.46:1。上方四張卡片本來就是不透明白底，不受影響。

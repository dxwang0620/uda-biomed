...
遮罩用該卡自己的色票，由上而下 `0.96 → 0.93 → 0.82 → 0.45 → 0.12`。
**0.82 是文字區的下限，由對比度決定**：白字疊在「色票遮罩 ＋ 純白照片」上
（`card1-1` 與 `card3-1` 都有大片近白的區域，這是最不利的情況）——

| 遮罩不透明度 | 01 `#1b42a3` | 02 `#313b87` | 03 `#0c132f` |
| ---------- | ----------- | ----------- | ----------- |
| 0.70 | 4.15 ✗ | 4.34 ✗ | 6.85 |
| **0.82** | **5.6** | **5.9** | **10.4** |
| 0.90 | 6.94 | 7.54 | 14.01 |

卡片高度是照片能不能被看到的關鍵。第一版 384 高時文字就佔了 346，
**照片只露 0～38px，等於沒有**；文案指定全部保留，所以只能把卡片加高：
...
| | 順序 |
| - | ---- |
| 窄螢幕 | hero → 消息 → 研發卡片 → 結語 → 四格數字 → 董事長談話 → 技術平台 → … |
| ≥1024 | hero → 第一列「消息｜研發卡片」→ 結語 → 四格數字 → 董事長談話 → … |

...
**刻意做柔**，讓它讀成「設計過的柔焦背景」而不是「畫質很差的影片」：

- 影片 `filter` 末端加 `blur(2.5px)`，並 `transform: scale(1.04)` 蓋掉模糊在
  四邊造成的透明羽化
- `.heroScrim::after` 疊一層極淡的顆粒（SVG `feTurbulence`，`opacity: 0.14`、
  `mix-blend-mode: overlay`）。純柔焦會像失焦的照片，加了顆粒之後會變成
  「有顆粒的影像」，柔就顯得像是選擇

顆粒是**靜態貼圖**，不像 filter 要逐幀計算；而且掛在 `.heroScrim` 的偽元素上，
不是影片上——影片有 filter，偽元素會跟著被 blur 掉（這個坑先前已經踩過一次）。

> **拿到高解析度原始檔之後**：把 `blur(2.5px)`、`transform: scale(1.04)` 與
> `.heroScrim::after` 三處一起移除，其餘不必動。

另外兩個當時評估過但沒採用的選項：影片改放在較窄的容器（放大倍率降到 1.2×，
但 hero 會從滿版動態變成有框的影片）、或退回 `dh-main.jpg` 靜態圖（1254×1254、
放大 1.5×，但沒有動態）。

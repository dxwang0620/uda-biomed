...
2026-09-11 收到正式文案。原本是「九張圖各配一句我自己寫的敘述」，
**那九段全部移除**，改成你的五大類，九張圖分配到各類底下當情境示意。
...
> 一套嗅覺系統，多種應用場景
>
> AFL 的技術發展方向，可延伸至不同產業。
>
> ・MEDICAL 醫療與健康：分析人體呼吸與揮發性生物標誌物，探索非侵入式氣味分析的可能性。
> ・FOOD 食品安全：從食品的新鮮度、品質變化，到異常氣味與污染特徵，建立非破壞性的氣味監測可能性。
> ・ENVIRONMENT 環境監測：持續感知空氣中的異常氣味與化學訊號，應用於室內、工業與環境監測。
> ・SAFETY 安全防護：針對危險氣體、異常化學物質與特定氣味模式建立偵測模型，協助系統提前發現異常。
> ・SECURITY 智慧安全：將氣味感知與 AI 分析結合，探索毒品、危險物質及其他特殊氣味的辨識應用。

### 圖片分配

依指示**每類一張圖**（2026-09-15）。整張圖鋪滿卡片、單一色相的漸層、
文字壓在上面（與研發焦點那三張卡同一套語彙）。

| 類別 | 圖 |
| --- | --- |
| MEDICAL 醫療與健康 | `dh-1` 居家呼氣檢測 |
| FOOD 食品安全 | `dh-2` 智慧冰箱 |
| ENVIRONMENT 環境監測 | `dh-6` 太空艙 |
| SAFETY 安全防護 | `dh-4` 住宅瓦斯與電線 |
| SECURITY 智慧安全 | `dh-9` 機場安檢 |

`dh-3`（隨身配戴）、`dh-5`（消防救援）、`dh-7`（零售）、`dh-8`（居家陪伴）
目前沒有用到，檔案留著隨時可換。

> ⚠️ **窄螢幕的停點要另外算，方向不變。**
> 兩個斷點都是「由左而右淡出、文字壓在左半」（指定）。差別只在淡出的位置：
> 窄螢幕的卡片才 339px 寬、文字欄佔掉 62%，若照抄桌機的停點（56% 還有 0.72），
> 右邊剩不到 100px 是亮的，照片等於沒放。窄螢幕整條往左壓：30% 之前實色、
> 50% 降到 0.90、68% 剩 0.62、86% 剩 0.22。
>
> 卡片高度：窄螢幕不綁比例，改成 `min-height: 11rem` ＋ 內容撐高（實測 213–262px）。
> 欄寬只有 210px，行數會隨文案長短差一兩行，綁死比例不是留一塊空白就是把字擠出卡片外。
> 768 以上才是 `aspect-ratio: 21 / 9`、文字收在左半 46%。
>
> 對比（白字，假設照片在文字後方是純白的最壞情況）：
> 窄螢幕文字右緣落在漸層 57% 處（濃度 0.79）＝ **9.35:1**；
> 桌機文字右緣在 58% 處（濃度約 0.70）＝ **6.69:1**。
>
> 走過的兩條岔路：第一版沿用桌機的停點 → 整張卡幾乎全暗；
> 第二版改成「由下而上、文字壓在下半、卡片 3:4」→ 照片是看得到了，
> 但卡片太高且文字不在左邊，不是要的樣式（回報「不要這麼長的圖片，我要漸層左邊是文字」）。

### 翻譯上的選擇

- 「可能性」在兩段裡出現（探索…的可能性／建立…的可能性），英文若照字面
  重複 `possibility` 會很拗口，改成 `exploring what … might make possible`
  與 `opening up …`
- 「協助系統提前發現異常」→ `so that a system can notice a problem early`

> ⚠️ **圖片尺寸用 `auto-fill` 不能用 `auto-fit`。**
> `auto-fit` 會把空軌道收掉，只有一張圖的類別（食品／環境）就會吃滿整個右欄，
> 跟旁邊兩張圖的類別大小差一倍。`auto-fill` 保留軌道，九張圖都是同一個尺寸
> （實測 224×168）。

### 仍是我擬的部分

只剩 `How it works` 的導言與四步說明。

## 影片在大螢幕上的模糊

回報「大尺寸的影片這麼模糊」。量過之後**原因是原始檔解析度，不是轉檔參數**：

| | 值 |
| --- | --- |
| 原始影片 | 960×720（裁掉浮水印後 **896×658**） |
| 桌機顯示 | 1920×544 |
| `object-fit: cover` 放大倍率 | **2.14×**（由寬度決定） |
| 實際算圖尺寸 | 1920×1410（只看得到垂直方向的 **39%**） |
| `devicePixelRatio` 2 時 | 等於 **4.3× 放大** |

896 的素材要撐到 3840，**沒有任何編碼設定救得回來**——細節在原始檔裡就不存在。
重轉、提高 bitrate、換 codec 都無效。

### 真正的解法

跟做這支動畫的人要 **1920 寬以上的輸出**。這看起來是算圖產出的，重新輸出應該不難。

### 試過但已移除的折衷

曾經加上 `blur(2.5px)` ＋ `scale(1.04)` ＋ 一層 SVG 顆粒，讓它讀成
「刻意的柔焦背景」而不是「畫質很差的影片」，**依指示移除**（2026-09-11）。
影片現在維持原本的銳利度，糊就是糊，等高解析度原始檔。
...
第一版把定位交給 flex（第一顆靠左、最後一顆靠右、中間兩顆置中），
四格等寬之下圓心會落在 **0 / 37.5% / 62.5% / 100%**——中間兩顆太靠近彼此。
而藍線是線性推進到 0 / 33.3% / 66.7% / 100%，兩者對不起來
（實測圓心 5 / 127 / 212 / 335，間距 **122 / 85 / 123**）。

改成用與藍線同一條公式定位：第 i 顆在 `4.5px + (i/3) × (100% − 9px)`。
實測圓心 **4.5 / 114.5 / 224.5 / 334.5**、間距 **110 / 110 / 110**，
且捲到 1/3 時藍線末端剛好落在第二顆圓心（同為 114.5）。

> ⚠️ 放大最新那一顆時，**`translateX(-50%)` 必須跟 `scale` 寫在一起**——
> `transform` 是整個換掉的，只寫 `scale` 會把定位用的位移弄丟，圓點往右跳半顆。

`FocusStack` 沒有這個問題是因為它只有三顆：靠左／置中／靠右剛好就是
0 / 50% / 100%，與線性推進一致。**點數一多，那個巧合就不成立了。**

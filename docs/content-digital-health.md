...
> ⚠️ **漸層的方向要依斷點換。**
> 窄螢幕若沿用桌機的「由左而右」，339px 寬的卡片上文字佔掉左邊 58%，
> 而漸層在那個範圍還接近實色——**整張卡幾乎全暗，照片等於沒放**（第一版就是這樣）。
> 手機改成由下而上、文字壓在下半（卡片 3:4），上半就留給照片；
> 768 以上才是由左而右、文字收在左半 46%（卡片 21:9）。
>
> 對比：桌機文字最右緣落在漸層 58% 處（濃度約 0.70），即使照片在那裡是純白，
> 白字仍有 **6.69:1**；手機文字起點在 60% 處，濃度 0.91，更高。

...
頁面最下層（`Our Story` / `Team` 佔位之後、CTA 帶之前）加入董事長談話，
**與首頁是同一個元件、同一份內容**（`ChairmanMessage`，2026-09-06 依指示加入）。

這一頁用 `tone="solid"`（深藍實底、白字），不是首頁的半透明玻璃——
ABOUT 沒有背景影片，半透明白疊在白底上等於看不見卡片邊界；實底同時把這一區
從前面的白／淺灰區塊區隔出來。所在的 Section 用 `tone="tint"`。

談話全文仍是草稿，需要本人或客戶確認；細節見 `docs/content-home.md`。

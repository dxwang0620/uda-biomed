...
- 樣式就是 RESEARCH 那顆 `Explore our technology →`（指定「那樣就好，簡單」）：
  同一個 `ArrowLink`，**不包 `<Section>`**——那會給它一整格的上下內距
  （桌機 6rem×2），一條連結佔掉一整個區塊的高度太刻意。
- 版位是**靠右 ＋ 上方一條 1px `--color-border` 細線**（指定），像文章的「下一篇」。
  細線與內容等寬（這一塊自己就是 `.container`，兩端對齊內容欄而不是視窗邊緣）。
  上 24px、下 48px，與細線之間再留 48px。
- 走過三版：`--color-tint` 色帶配大字標題（太重）→ 包在 `<Section>` 裡的
  置中 ArrowLink（一整格太刻意）→ 不含 Section 的置中版（在頁尾 CTA 底下
  變成第二個視覺焦點，跟 CTA 打架）→ 現在的細線靠右。
- 前面加一段看得見的 **`NEXT:`**（指定改成斜體）。原本是唯讀文字——
  只唸標題的話，離開上下文就不知道它要去哪，現在畫面上也看得到。
  **整條連結都斜體**，包含中文標題（指定「其他字也斜體」）。
  ⚠️ 中文沒有真正的義大利體，瀏覽器是把正體硬傾斜（synthetic oblique），
  筆畫會跟著歪。這件事回報過，仍指定要斜體，所以照做。
  選擇器下在 `.cta` 底下的連結上，不是改 `ArrowLink`——那顆元件還有別處在用。

...
export default function Header() {
  ...
  /** 手機選單裡展開中的那一組子項目，值是父項目的 to；null＝都收起來 */
  ...
  /* 開啟手機選單時，把目前所在頁所屬的那一組展開。
     全部收起來的話，看不到自己在哪一頁底下——那正是 aria-current 要標的資訊。 */
  useEffect(() => {
    if (!menuOpen) return;
    ...
    setOpenSub(parent ? parent.to : null);
  }, [menuOpen, pathname]);
            ...
            {NAV_ITEMS.map((item) =>
              "children" in item && item.children ? (
                <PanelGroup
                  key={item.to}
                  item={item}
                  open={openSub === item.to}
                  onToggle={() =>
                    setOpenSub((cur) => (cur === item.to ? null : item.to))
                  }
                />
              ) : (
 ...
 *
 * 收合用 grid-template-rows 由 0fr 轉 1fr：這是少數能對「高度 auto」做轉場的
 * 做法，用 max-height 猜一個值會在項目數改變時卡頓或截斷。
...
function PanelGroup({
  ...
  const id = `panel-sub-${item.to.replace(/\W+/g, "-")}`;
  ...
  return (
    <div className={styles.panelGroup}>
      <div className={styles.panelRow}>
        ...
        >
          <span className="visually-hidden">
            {open ? `收合 ${item.label} 子選單` : `展開 ${item.label} 子選單`}
          </span>
          <ChevronDown
            size={20}
            strokeWidth={2.5}
      ...
      <div
        id={id}
        className={`${styles.panelCollapse} ${open ? styles.panelCollapseOpen : ""}`}
      >
  ...
  return (
    <div
      ...
      /* 滑鼠移入即展開；移出整塊才收。焦點離開整塊也收，
         這樣用鍵盤 Tab 出去時選單不會留著。 */
      onPointerEnter={
        onToggle === undefined ? undefined : () => !open && onToggle()
      }
